    ...
    def check_service(self, status, which, notes=""):
        ...
        if self.checked_family in ("redhat", "suse"):
            if os.path.exists("/etc/rc.d/init.d/%s" % which):
                rc = utils.subprocess_call(self.logger, "/sbin/service %s status > /dev/null 2>/dev/null" % which,
                                           shell=True)
            if rc != 0:
            ...
            if os.path.exists("/etc/init.d/%s" % which):
                rc = utils.subprocess_call(self.logger, "/etc/init.d/%s status /dev/null 2>/dev/null" % which,
                                           shell=True)
            if rc != 0:
                status.append("service %s is not running%s" % (which, notes))
                return
